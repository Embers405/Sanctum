...
/**
 * @author Luke @ Major credits to the origianl creator who ever they maybe <3
 */
...
public class npc_drops extends DataFile {
    ...
    public Object fromJson(String fileName, String json) {
        ...
        loaded.forEach((id, table) -> {
            NPCDef npcDef = NPCDef.get(id);
            if (npcDef != null) {
                npcDef.lootTable = table;
            } else {
                System.err.println("Could not find NPC def " + id);
            }
        });
        return tables;
    }
        ...
        public DumpTable run() {
            try {
                ...
                for (Element header : tableHeaders) {
                    ...
                    tableName = tableName.replace("100% drop", "Always").replaceFirst("100%", "Always").replace("Alwayss",
                            "Always");
                    Elements trs = dl.select("table > tbody > tr");
                    if (trs == null)
                        continue;
            ...
            try {
                ...
                for(String s : split) {
                    ...
                    if(s.equalsIgnoreCase("unknown")) {
                        min = 1;
                        max = 1;
                        itemNotes = new ArrayList<>(itemNotes);
    ...
    private static final class DumpTable extends LootTable {
        ...
        public void export(String wikiName, Integer[] ids) {
            File file = FileUtils.get("%HOME%/Desktop/Revenant_NPC_DATA/Server/data/wiki_dumps/drops/" + wikiName + ".json");
