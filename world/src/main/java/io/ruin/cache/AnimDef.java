...
public class AnimDef {
    ...
    void readValues(InBuffer buffer, int opcode) {
        if(opcode == 1) {
            ...
            int var4 = buffer.readUnsignedByte();
            chatFrameIds = new int[var4];
            for(int var5 = 0; var5 < var4; var5++)
                chatFrameIds[var5] = buffer.readUnsignedShort();
            for(int var5 = 0; var5 < var4; var5++)
                chatFrameIds[var5] += buffer.readUnsignedShort() << 16;
        } else if(opcode == 13) {
            int var4 = buffer.readUnsignedByte();
            frameSounds = new int[var4];
            for(int var5 = 0; var5 < var4; var5++)
                frameSounds[var5] = buffer.readMedium();
        }
          //else
          //  System.err.println("MISSING ANIM OPCODE " + opcode + " FOR ID " + id);
    }
