...
import io.ruin.model.skills.agility.GracefulSet;
...
public class StatList {
    ...
    public void addXp(StatType type, double amount, boolean useMultiplier) {
        ...
        if (type == StatType.Agility) {
            amount = (amount * GracefulSet.GracefulSet(player));
        }
         ...
         */
        if(player.SOTRR && player.getInventory().hasItem(26071, 1) && type == StatType.Ranged) {
            amount *= 2.00;
        }

        if(player.SOTMM && player.getInventory().hasItem(26077, 1) && type == StatType.Magic) {
            amount *= 2.00;
        }
        /*
        ...
        if(newXp == Stat.MAX_XP && stat.experience < Stat.MAX_XP) {
            if (!player.getGameMode().isIronMan()) {
                Broadcast.GLOBAL.sendNews(Icon.NORMAL,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            } else if (player.getGameMode().isHardcoreIronman()) {
                Broadcast.GLOBAL.sendNews(Icon.HCIM,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            } else if (player.getGameMode().isUltimateIronman()) {
                Broadcast.GLOBAL.sendNews(Icon.UIM,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            } else if (player.getGameMode().isGroupIronman()) {
                Broadcast.GLOBAL.sendNews(Icon.GIM,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            } else if (player.getGameMode().isHardcoreGroupIronman()) {
                Broadcast.GLOBAL.sendNews(Icon.HGIM,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            } else {
                Broadcast.GLOBAL.sendNews(Icon.IRONMAN,"<col=ff0000>[" + player.xpMode.getCombatRate() + "x] </col>" + player.getName() + " has just reached 200 million experience in " + type.name() + "!");
            }
