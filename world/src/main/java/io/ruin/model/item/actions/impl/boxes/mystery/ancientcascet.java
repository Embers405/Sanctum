...
public class ancientcascet {
    public static final LootTable ANCIENT_TABLE = new LootTable().addTable(1,
            ...
            new LootItem(30248,1,2).broadcast(Broadcast.WORLD)
    );
    ...
    static {
        ItemAction.registerInventory(23071, "open", (player, item) -> {
            ...
            Item reward;
            reward = ANCIENT_TABLE.rollItem();
            item.remove(1);
            player.getInventory().add(reward);
            if (reward.lootBroadcast != null)
                Broadcast.WORLD.sendNews(Icon.MYSTERY_BOX, "ancient Box", "" + player.getName() + " just received " + reward.getDef().descriptiveName + "!");
            player.unlock();
        });
         ...
         * Mystery box gifting
         */
        ItemAction.registerInventory(6199, "gift", ancientcascet::gift);
    }
}
