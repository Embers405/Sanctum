...
public class HolyScytheOfVitur {
    ...
    private static void uncharge(Player player, Item item) {
        int reqSlots = 0;
        int bloodAmount = item.getUniqueValue();
        if (bloodAmount > 0) {
    ...
    private static void check(Player player, Item item) {
        if (item.getId() == SCYTHE) {
            player.sendMessage("Your Scythe of Vitur " + item.getUniqueValue() + " charges remaining.");
        }
    }

    private static void charge(Player player, Item scythe, Item blood) {
        int currentCharges = scythe.getUniqueValue();
        ...
        scythe.setUniqueValue(currentCharges + (addAmount));
        scythe.setId(SCYTHE);
        check(player, scythe);
    }
}
