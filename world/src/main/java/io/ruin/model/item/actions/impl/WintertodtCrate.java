...
public class WintertodtCrate {
    ...
    public static final LootTable table = new LootTable().addTable(1,
            ...
            new LootItem(6739, 1, 1, 1)            // dragon axe
    );

    private static final int[] RARE_ITEMS = {20704, 20706, 20708, 20710, 20712, 20716, 20720};
    private static int rareitem = 0;
    static {
        ItemAction.registerInventory(20703, "open", (player, item) -> {
            if(Random.rollDie(64, 1)) {
                rareitem = RARE_ITEMS[Random.get(RARE_ITEMS.length - 1)];
                player.getInventory().addOrDrop(rareitem, 1);
                player.getCollectionLog().collect(rareitem);
            }
            item.remove();
            ...
            player.getCollectionLog().collect(firstReward, secondReward);
        });
    }
}
