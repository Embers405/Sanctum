...
import io.ruin.model.item.actions.impl.petperk.impl.*;
...
public enum Pet {
    ...
    SNAKELING_GREEN(12921, 2130, 2131, true, 1000, new VenomPoisonImmunityPerk()),
    SNAKELING_RED(12939, 2131, 2132, false, new VenomPoisonImmunityPerk()),
    SNAKELING_BLUE(12940, 2132, 2130, false, new VenomPoisonImmunityPerk()),
    ZILYANA(12651, 6633, true, 1000),
    PRINCE_BLACK_DRAGON(12653, 6636, true, 1000, new DragonFireImmunityPerk()),
    ...
    ABYYSALSIRE(30047, 5883, 5884, true,1000),
    CERBERUS(30045, 964, 3099, true,1000),
    NEXLING(30367,14760,true,1000),
    LIL_CREATOR(25348,2833, false, 2000, new BonusStrengthPerk(){
        @Override
        public int getBonusStrength(Player player, StatType statType) {
            if (statType.equals(StatType.Attack)) {
                ...
                if (itemDef.weaponType == null)
                    return 0;

                WeaponType claws = weapon_types.MAP.get("CLAWS");

                boolean hasclaws = itemDef.weaponType.equals(claws);

                return hasclaws ? 10 : 0;
            }

            return 0;
        }
    }),
    LITTLE_NIGHTMARE(24491, 9398, false, 2000, new BonusSmitePerk() {
        @Override
        public double getBonusSmitePercentage(Player player) {
            return 10;
        }
    }),
    LIL_DESTRUCTOR(25350, 3564, false, 2000, new BonusStrengthPerk() {
        @Override
        public int getBonusStrength(Player player, StatType statType) { //this bonus strength effects both melee and ranged
...
//                System.out.println(itemDef);
                ...
                WeaponType two_handed = weapon_types.MAP.get("TWO_HANDED");

                boolean has2h = itemDef.weaponType.equals(two_handed);
    ...
    TINY_TEMPOR(25602, 10562, 10637, false,200),
    KANGALI(30117, 14687, false),
    MALLICE(30118, 14689, false),
    MORTY(30119, 14691, false),
    SPARKY(30120, 14693, false),
    GHOSTLY(30121, 14695, false),
    NEWTON(30122, 14697, false),
    GALVEK_JR(30236, 15014, false),
    SERENITY(30231, 15086, false),

    /* *
     * Skilling pets
     * */
    BABY_CHINCHOMPA_GREY(13324, 6757, 6756, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    BABY_CHINCHOMPA_RED(13323, 6756, 6758, false, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BABY_CHINCHOMPA_BLACK(13325, 6758, 6757, false, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BABY_CHINCHOMPA_GOLD(13326, 6759, 6757, false, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BEAVER(13322, 6724, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Woodcutting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    HERON(13320, 6722, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Fishing) { //checks if it is the skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    ROCK_GOLEM(13321, 7451, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Mining) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    GIANT_SQUIRREL(20659, 7351, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Agility) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    TANGLEROOT(20661, 7352, false , new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Farming) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    ROCKY(20663, 7353, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Thieving) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    RIFT_GUARDIAN_FIRE(20665, 7354, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    RIFT_GUARDIAN_AIR(20667, 7355, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_MIND(20669, 7356, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_WATER(20671, 7357, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_EARTH(20673, 7358, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_BODY(20675, 7359, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_COSMIC(20677, 7360, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_CHAOS(20679, 7361, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_NATURE(20681, 7362, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_LAW(20683, 7363, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_DEATH(20685, 7364, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_SOUL(20687, 7365, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_ASTRAL(20689, 7366, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_BLOOD(20691, 7367, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    HERBI(21509, 7760, true, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Herblore) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
            }
            return 0; //other skills will not have a exp boost
        }
    }),
    PACK_YAK(30010, 11389, true),
     ...
     * Golems
     */
    TIN_GOLEM(21187, 7440, true),
    CLAY_GOLEM(21188, 7441, true),
    IRON_GOLEM(21189, 7442, true),
    BLURITE_GOLEM(21190, 7443, true),
    SILVER_GOLEM(21191, 7444, true),
    COAL_GOLEM(21192, 7445, true),
    GOLD_GOLEM(21193, 7446, true),
    MITHRIL_GOLEM(21194, 7447, true),
    ADAMANT_GOLEM(21196, 7449, true),
    RUNITE_GOLEM(21197, 7450, true),
    ...
    WALKER(30291, 14745, false),
    DEATH_PET(30302, 14746, false, 2000);
    ...
    public boolean hasPerk(PerkType perkType) {
        return perks.stream()
                .anyMatch(p -> p.perkType().equals(perkType));
    }

    public <T> T getPerk(PerkType perkType, Class<T> perkClass) {
        Perk perk = perks.stream()
                .filter(p -> p.perkType().equals(perkType))
                .findFirst().orElse(null);
        return perkClass.cast(perk);
    }

    Pet(int itemId, int npcId, boolean mysteryBox, Perk... perks) {
        this(itemId, npcId, -1, mysteryBox, 0, perks);
    }

    Pet(int itemId, int npcId, int metaId, boolean mysteryBox, Perk... perks) {
        this(itemId, npcId, metaId, mysteryBox, 0, perks);
    }

    Pet(int itemId, int npcId, boolean mysteryBox, int dropAverage, Perk... perks) {
        this(itemId, npcId, -1, mysteryBox, dropAverage, perks);
    }

    Pet(int itemId, int npcId, int metaId, boolean mysteryBox, int dropAverage, Perk... perks) {
        this.itemId = itemId;
        this.npcId = npcId;
        this.metaId = metaId;
        ...
        this.perks = new ArrayList<>(Arrays.asList(perks));
    }


    private int findRoamId() {
        NPCDef baseDef = NPCDef.get(npcId);
        if (baseDef == null || baseDef.name == null) {
            System.err.println("INVALID PET " + this + " (" + npcId + ")");
            return -1;
        }
    ...
    public void spawn(Player player) {
        ...
        npc.addEvent(e -> {
            while (player.isOnline()) {
                ...
                if (player.callPet || !npc.getPosition().isWithinDistance(player.getPosition(), 10)) {
    ...
    static {
        for (Pet pet : values()) {
            ...
            itemDef.pet = pet;
            NPCDef npcDef = NPCDef.get(pet.npcId);
            if (npcDef == null) {
                continue;
            }
    ...
    private static Consumer<Player> getTalkAction(Pet pet) {
        switch (pet) {
            ...
            case LIL_DESTRUCTOR:{
                return player -> player.dialogue(new PlayerDialogue("Don't you get hot standing around pools of lava all day?"),
                        new NPCDialogue(pet.npcId,  "I care not for equable temperature... only destruction!"),
                        new PlayerDialogue("Each to their own, I suppose..."));
            }
            case LIL_CREATOR: {
                return player -> player.dialogue(new PlayerDialogue("So why are you fighting over that obelisk anyway? Just looks like a bit of old rock to me."),
                        new NPCDialogue(pet.npcId,  "Puny human! You wouldn't understand."),
                        new PlayerDialogue("Yeah, I'm the puny one."));
            }
        }
        return null;
    }
