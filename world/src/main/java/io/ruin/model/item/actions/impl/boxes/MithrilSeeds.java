...
public class MithrilSeeds {
    ...
    private static void plant(Player player, Item item) {
        Bounds GamblingZone = new Bounds(1737,3613,1749,3628,0);
        if (!Tile.allowObjectPlacement(player.getPosition()) && !player.getPosition().inBounds(GamblingZone)) {
        ...
        player.startEvent(event -> {
            ...
            Tile Flower = Tile.get(obj.getPosition());
            Flower.flagUnmovable();
            player.getRouteFinder().routeSelf(true);
            event.waitForMovement(player);
            player.face(obj);
            player.unlock();
            player.getDiaryManager().getKourendDiary().progress(KourendDiaryEntry.MITHRIL_SEED);
            World.startEvent(worldEvent -> {
                worldEvent.delay(60);
                if (!obj.isRemoved()) {
                    obj.remove();
                    Flower.unflagUnmovable();
                }
            });
        });
    }
