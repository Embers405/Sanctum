...
public class GIMStorage extends ItemContainerG<GIMItem> {
    ...
    public boolean sendUpdates(ItemContainerG mirrorContainer) {
        ...
        player.getGroupIronman().saveData.get().players.forEach(p -> {
            if(sendAll) {
                if(p != null) {
                    p.getPacketSender().sendItems(interfaceHash, containerId, items, items.length);
                }
            } else {
                if(p != null) {
                    p.getPacketSender().updateItems(interfaceHash, containerId, items, updatedSlots, updatedCount);
                }
            }
        });
