    ...
    static {
        for (Potions potion : Potions.values()) {
            if (potion != Potions.OVERLOAD) {
                ...
                ItemItemAction.register(potion.secondaryId, 20801, (player, herbId, gourdVial) -> onGourdVial(player, potion, -1));// cameron come back to this if dont fix poitions for raids
            } else {
                for (int i = 0; i < 3; i++) {
