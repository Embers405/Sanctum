...
public class GreatOlm extends NPCCombat {
    ...
    static {
        ObjectDef.get(32297).isClippedDecoration(); // force flame wall fire to clip tiles
        ObjectDef.get(32297).gateType = true;
        ...
        npc.addEvent(event -> {
            ...
            while (ticks < duration) {
                for (int i = 0; i < 2; i ++) {
                    ...
                    World.sendGraphics(1447, 0, 30, position);
    ...
    private void nextPhase() {
        ...
        if (currentPhase >= lastPhase) {
            if (!finalStand) {
                finalStand = true;
                ceilingCrystals(0, 2500); // Was 10k
    ...
    private void sphereAttack(List<Player> targets) {
        ...
        for (int i = 0; i < 3 && targets.size() > 0; i++) {
            ...
            if (target.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES) || target.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC) || target.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE)) {
                target.getPrayer().drain(target.getStats().get(StatType.Prayer).currentLevel / 2);
//                target.getPrayer().deactivateProtectionPrayer(); //TODO Fix the method for deactivating prayers
    ...
    private void flameWall() {
        ...
        npc.addEvent(event -> {
            ...
            if (isOnEastSide()) {
                for (int x = projectileX; x < projectileX + 10; x++) {
                    fires.add(GameObject.spawn(32297, x, targetY + 2, 0, 10, 0));
                    fires.add(GameObject.spawn(32297, x, targetY - 2, 0, 10, 0));
                }
            } else {
                for (int x = projectileX; x > projectileX - 10; x--) {
