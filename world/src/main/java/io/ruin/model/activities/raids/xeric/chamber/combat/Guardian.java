...
public class Guardian extends NPCCombat {
    ...
    private void meleeAttack() {
        ...
        target.hit(new Hit(npc, AttackStyle.SLASH, AttackType.AGGRESSIVE).randDamage(minDamage, maxDamage).ignoreDefence().ignorePrayer());
    }
        ...
        npc.addEvent(event -> {
            ...
            npc.localPlayers().forEach(p -> {
                if (p.getPosition().equals(pos)) {
                    target.hit(new Hit(npc, AttackStyle.RANGED, null).fixedDamage(target.getHp() / 3).ignoreDefence().ignorePrayer());
                    wait(100);
                }
            });
        });
