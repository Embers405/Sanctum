...
public class Zulrah extends NPCCombat { // this is a mess
    ...
    static {
        ...
        ObjectAction.register(10068, "quick-board", (player, obj) -> {
            enter(player);
        });
    ...
    public static void enter(Player player) {
        player.startEvent(event -> {
            ...
            map.assignListener(player).onExit((p, logout) -> {
                ...
                p.getCombat().isDefending(0);
                p.getCombat().isAttacking(0);
                map.destroy();
            });
            event.delay(2);
    ...
    private void magicAttack() {
        ...
        Hit hit = new Hit(npc, AttackStyle.MAGIC).randDamage(maxdmg).clientDelay(duration);
        target.hit(hit);
    }
    ...
    private void rangedAttack() {
        ...
        Hit hit = new Hit(npc, AttackStyle.RANGED).randDamage(maxdmg).clientDelay(duration);
        target.hit(hit);
    }
    ...
    private void meleeAttack() {
        ...
        npc.addEvent(event -> {
            ...
            if (Misc.getDistance(meleeTarget, target.getPosition()) <= 1) {
                ...
                target.hit(new Hit(npc, AttackStyle.STAB).randDamage(maxdmg));
