...
//                    new LootItem(13574, 55, 1), // dynamite
    ...
    public static void giveRewards(ChambersOfXeric raid) {
        ...
        for (int i = 0; i < 3; i++) { // up to 3 uniques
            ...
            double chance = pointsToUse / 4200 / 100.0; // 1% chance per 4200 points - OSRS = 8675
            if (Random.get() < chance) {
                ...
                Broadcast.GLOBAL.sendNews(Color.RAID_PURPLE.wrap("[RARE DROP] ")+lucker.getName()+" Has just received "+Color.DARK_RED.wrap(item.getDef().name)+" from Chambers of Xeric!");
            }
        }
        //regular drops
