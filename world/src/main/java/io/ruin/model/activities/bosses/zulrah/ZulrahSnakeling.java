...
public class ZulrahSnakeling extends NPCCombat {
    ...
    @Override
    public void init() {
        style = Random.get() < 0.4 ? AttackStyle.MAGIC : AttackStyle.SLASH;
        npc.attackBounds = npc.walkBounds = new Bounds(npc.getSpawnPosition(), 32);
    }

    @Override
    public void follow() {
        follow(style == AttackStyle.MAGIC ? 6 : 1);
    }
    ...
    public boolean attack() {
        if (!withinDistance(style == AttackStyle.MAGIC ? 6 : 1))
            return false;
        if (style == AttackStyle.MAGIC) {
            npc.animate(info.attack_animation);
            int duration = PROJECTILE.send(npc, target);
            target.hit(new Hit(npc, AttackStyle.MAGIC).randDamage(1, 10).clientDelay(duration));
        } else {
            basicAttack();
        }
        if (Random.get() < 0.15)
