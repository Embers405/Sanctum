        ...
        npc.deathEndListener = (DeathListener.Simple) () -> {
            for (Player player : npc.localPlayers()) {
                for (int i = 0; i < 2; i++) {
                    ...
                    if (rolled.getId() == 30256 || rolled.getId() == 6829 || rolled.getId() == 30250 || rolled.getId() == 30248 || rolled.getId() == 30257 || rolled.getId() == 11944) {
                        Broadcast.GLOBAL.sendNews("<shad=000000>" + Color.RAID_PURPLE.wrap("[RARE DROP] ") + player.getName() + " has just received " + Color.DARK_RED.wrap(rolled.getDef().name) + " from The Vote Boss!" + "</shad>");
                    }
                }
            }
            npc.remove();
...
/*        boolean healed = false;
    ...
    public static LootTable regularTable = new LootTable()
            ...
            .addTable(10,
                    new LootItem(30256, Random.get(1, 3), 10), // Vote Multipass
                    new LootItem(6829, 1, 7), // Vote Box
                    new LootItem(30250, 1, 1), // $10 Bond
                    new LootItem(30248, 1, 1), // $5 Bond
                    new LootItem(25348,1,1),
