...
public class UnderwallTunnel {
    ...
    public static void shortcutN(Player p, GameObject wall, int levelReq) {
        if (!p.getStats().check(StatType.Agility, levelReq, "attempt this"))
            return;
        p.startEvent(e -> {
            if(wall.id == 16527) {
                p.lock(LockType.FULL_DELAY_DAMAGE);
                p.animate(2589);
                p.getMovement().force(0, 2, 0, 0, 0, 50, Direction.NORTH);
                e.delay(2);
                p.animate(2590);
                p.getMovement().force(0, 1, 0, 0, 0, 100, Direction.NORTH);
                e.delay(3);
                p.animate(2591);
                ...
                p.lock(LockType.FULL_DELAY_DAMAGE);
                p.animate(2589);
                p.getMovement().force(0, -2, 0, 0, 0, 50, Direction.SOUTH);
                e.delay(2);
                p.animate(2590);
                p.getMovement().force(0, -1, 0, 0, 0, 100, Direction.SOUTH);
                e.delay(3);
                p.animate(2591);
