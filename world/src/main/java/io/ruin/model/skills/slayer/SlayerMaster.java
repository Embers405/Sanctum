...
public class SlayerMaster {
    ...
    public static final int[] IDS = {6797, 7663, 9149, 8623, 401, 403, 405};
    ...
    public static final int CRYSTALLINE = 9149;
    ...
    static {
        ...
        NPCAction.register(CRYSTALLINE, "Talk-to", (player, npc) -> player.dialogue(
                new NPCDialogue(npc, "'Ello, and what are you after then?"),
                new OptionsDialogue(
                        player.slayerTask == null ?
                                new Option("I need another crystal assignment.", () -> {
                                    player.dialogue(
                                            new PlayerDialogue("I need another CRYSTAL assignment."),
                                            new ActionDialogue(() -> assigncrystal(player, npc))
                                    );
                                }):
                                new Option("Tell me about my Slayer assignment.", () -> {
                                    player.dialogue(
        ...
        NPCAction.register(CRYSTALLINE, "Assignment", SlayerMaster::assigncrystal);
        NPCAction.register(CRYSTALLINE, "Trade", (player, npc) -> ShopManager.openIfExists(player, "06bf1d5a-e4d8-49f5-9aac-6229a965b98a"));
        NPCAction.register(CRYSTALLINE, "Rewards", (player, npc) -> rewards(player));
    }
    ...
    private static void assigncrystal(Player player, NPC npc) {
        if (player.currentSlayerMaster == 0 && Slayer.getTask(player) == null) {
            player.dialogue(new NPCDialogue(npc, "Would you like me to assign your first slayer task? I assign CRYSTAL tasks."),
                    new OptionsDialogue(
                            new Option("Yes please.", () -> player.dialogue(
                                    new PlayerDialogue("Yes please."),
                                    new ActionDialogue(() -> {
                                        set(player, SlayerTask.Type.CRYSTAL);
                                        player.currentSlayerMaster = CRYSTALLINE;
                                        player.dialogue(new NPCDialogue(npc, "Your new task is to kill " + player.slayerTaskRemaining + " " + player.slayerTaskName + ".<br>Good luck!").lineHeight(28));
                                    }))),
                            new Option("No, Thanks.")
                    )
        ...
        if (player.currentSlayerMaster == CRYSTALLINE && Slayer.getTask(player) != null) {
            player.dialogue(new NPCDialogue(npc, "You already have an CRYSTAL task, Go kill " + player.slayerTaskRemaining + " more " + player.slayerTaskName));
            return;
        }
        if (player.currentSlayerMaster != CRYSTALLINE && Slayer.getTask(player) != null) {
            player.dialogue(new NPCDialogue(npc, "If you would like an easier task, I will need to reset your task streak."),
