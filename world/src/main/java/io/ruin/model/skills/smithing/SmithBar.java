...
/*    CORRUPT(
    ...
    private static void open(Player player) {
        ...
        if(bar == null/* || bar.equals(SmithBar.CORRUPT)*/) {
            player.dialogue(new MessageDialogue("You don't have any bars in your inventory to smith with!"));
            return;
        }
        open(player, bar);
    }
        ...
        InterfaceHandler.register(Interface.SMITHING, h -> {
            ...
            for(int i = 9; i <= 35; i++) {
                ...
                h.actions[i] = (DefaultAction) (p, option, slot, itemId) -> {
                    ...
                    if(amount.get() == 4) {
                        p.integerInput("Enter amount:", amt -> item.make(p, amt));
                        return;
                    } else
                    if(amount.get() == 5) {
                        item.make(p, Math.max(1, p.getInventory().getAmount(p.smithBar.itemId) / item.barReq));
                        return;
                    } else {
                        item.make(p, Math.max(1, p.getInventory().getAmount(p.smithBar.itemId) / item.barReq));
                    }
                    Item.examine(p, item.makeId);
                };
            }
        });
    }
