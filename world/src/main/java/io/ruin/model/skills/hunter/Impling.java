...
public enum Impling {
    ...
    ECLECTIC(1640, 50, 60, 30, 32, 11248, 30, 10),
    ...
    LUCKY(7233, 89, 99, 0, 0, 19732, 2, 2);
    ...
    private static final int PURO_PURO_MAX_RANDOM_IMPLINGS = 12; // maximum number of random spawns that can be active at a time
    private static final int PURO_PURO_TOTAL_SPAWN_WEIGHT = Arrays.stream(values()).mapToInt(imp -> imp.puroPuroSpawnWeight).sum();

    private static final int OVERWORLD_SPAWN_DELAY = 100; // spawn time for random spawns in the overworld (implings will attempt to spawn at this interval, if the active number is below maximum)
