...
import io.ruin.model.diaries.AchievementDiary;
...
//                player.sendMessage("<col=000080>You have completed the achievement: <col=800000>" + getListener().name());
                ...
                getListener().lines(player, true);
                System.out.println(getListener().name() + " Finished");
            }
        }
    }
...
//                player.sendScroll("<col=800000>" + getListener().name(), getListener().lines(player, isFinished(player)));
    ...
    public static void updateAchievementDiaryVarbits(Player player, int varbitId, int value)  {
        switch (varbitId) {
