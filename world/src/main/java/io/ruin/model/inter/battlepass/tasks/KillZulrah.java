...
public class KillZulrah implements AchievementListener {
    ...
    public void init(Player player, int setAmount) {
        amount = setAmount;
        xpReward = amount * 25;
        counterStartValue = player.zulrahKills.getKills();
    ...
    public int get(Player player) {
        counterStartValue = player.getBattlePass().tasks.Kill10Zulrah.counterStartValue;
        int i = player.zulrahKills.getKills() - counterStartValue;
        if(i < 0) {
            init(player,amount);
        }
        return i;
