...
public class KillBarrows implements AchievementListener {
    ...
    public void init(Player player, int setAmount) {
        amount = setAmount;
        xpReward = amount * 25;
        counterStartValue = player.barrowsChestsLooted.getKills();
    ...
    public int get(Player player) {
        counterStartValue = player.getBattlePass().tasks.loot10BarrowsChest.counterStartValue;
        int i = player.barrowsChestsLooted.getKills() - counterStartValue;
        if(i < 0) {
            init(player,amount);
        }
        return i;
