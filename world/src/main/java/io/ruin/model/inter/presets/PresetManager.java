...
//        player.getStats().set(StatType.Prayer, preset.getPrayerLevel());
        ...
        if (modifiedEquipment) {
            player.sendMessage(Color.RED + "Your preset equipment has been modified since you did not have all items in your bank.");
        }

        if (modifiedInventory) {
            player.sendMessage(Color.RED + "Your preset inventory has been modified since you did not have all items in your bank.");
        }
            ...
            if(foundItem == null) {
                ...
                if(amountInBank - item.getAmount() < 1 && Config.BANK_ALWAYS_PLACEHOLDERS.get(player) == 1) {
                    if (itemDef != null && itemDef.hasPlaceholder()) {
                        ...
                        player.getBank().remove(item.getId(), item.getAmount(), foundItem.copyOfAttributes());
                    }
                } else {
                    player.getBank().remove(item.getId(), item.getAmount(), foundItem.copyOfAttributes());
                }
