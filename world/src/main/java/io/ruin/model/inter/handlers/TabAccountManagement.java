...
public class TabAccountManagement {
    ...
    private static final String FORUM_INBOX_URL = "https://obsidian-ps.com/";
...
//            h.actions[82] = (SimpleAction) p -> p.getMovement().teleport(1747,3598,0);
            h.actions[83] = (SimpleAction) p -> p.openUrl("RuneRogue Hiscores", "https://obsidian-ps.com/");
            h.actions[84] = (SimpleAction) p -> p.openUrl("Runescape Wikipedia","https://oldschool.runescape.wiki");
            h.actions[85] = (SimpleAction) p -> p.openUrl("RuneRogue Store", "https://obsidian-ps.com/");
    ...
    public static String getDonatorRank(Player player) {
        if (player.isGroups(SecondaryGroup.DRAGON)) {
            return SecondaryGroup.DRAGON.tag() + " Godlike";
        } else if (player.isGroups(SecondaryGroup.RUNE)) {
            return SecondaryGroup.RUNE.tag() + " Master";
        } else if (player.isGroups(SecondaryGroup.ADAMANT)) {
            return SecondaryGroup.ADAMANT.tag() + " Ultimate";
        } else if (player.isGroups(SecondaryGroup.MITHRIL)) {
            return SecondaryGroup.MITHRIL.tag() + " Extreme";
        } else if (player.isGroups(SecondaryGroup.GOLD)) {
            return SecondaryGroup.GOLD.tag() + " Super";
        } else if (player.isGroups(SecondaryGroup.BRONZE)) {
            return SecondaryGroup.BRONZE.tag() + " Regular";
        } else {
            return "Unranked";
        }
    }
