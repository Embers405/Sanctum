package io.ruin.model.inter.battlepass.tasks;
...
public class MineMithrilOre implements AchievementListener {

    @Expose
    public int counterStartValue = 0;

    private static PlayerCounter counters = PlayerCounter.MINED_MITHRIL;
    ...
    private String oreName = "Mithril Ore";

    public MineMithrilOre(int i) {
        amount = i;
        xpReward = amount * 25;
    ...
    public void init(Player player, int setAmount) {
        amount = setAmount;
        xpReward = amount * 25;
        PlayerCounter.MINED_MITHRIL.increment(player,1);
        PlayerCounter.MINED_MITHRIL.decrement(player,1);

        counterStartValue = PlayerCounter.MINED_MITHRIL.get(player);
    ...
    public int get(Player player) {
        counterStartValue = player.getBattlePass().tasks.Mine10Mithril.counterStartValue;
    ...
    public String[] lines(Player player, boolean finished) {
        return new String[]{
                ...
                "",
                Achievement.slashIf("<col=000080>Assignment</col>: Mine " + amount + " "+ oreName, finished),
    ...
    public void finished(Player player) {
        player.getBattlePass().addExp(xpReward,player);
    }
}
