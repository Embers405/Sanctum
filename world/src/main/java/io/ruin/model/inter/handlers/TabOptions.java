...
public class TabOptions {
    ...
    static {
        InterfaceHandler.register(Interface.OPTIONS, h -> {
            ...
            h.actions[74] = (SimpleAction) p -> openBonds(p);
        ...
        if (false)
            InterfaceHandler.register(Interface.OPTIONS, h -> {
                ...
                h.actions[85] = (SimpleAction) (p) -> {
                    if (Config.SHIFT_DROP.get(p) == 0) {
                        Config.SHIFT_DROP.set(p, 1);
                        p.Shift_Drop = true;
                    } else {
                        Config.SHIFT_DROP.set(p, 0);
                        p.Shift_Drop = false;
                    }
                };
        ...
        static {
            InterfaceHandler.register(Interface.KEYBINDING, h -> {
                ...
                h.actions[103] = (SimpleAction) p -> {
                    if (Config.ESCAPE_CLOSES.get(p) == 0) {
                        Config.ESCAPE_CLOSES.set(p, 1);
                        p.ESC_Close = true;
                    } else {
                        Config.ESCAPE_CLOSES.set(p, 0);
                        p.ESC_Close = false;
                    }
                };
                /**
