...
public class DragonSlayer2 {
    ...
    private static void alecDialogue(Player player, NPC npc) {
        ...
        if (PlayerCounter.DRAGON_SLAYER_2.get(player) == 12) {
            player.dialogue(
                    new NPCDialogue(ALEC_KINCADE, "You've done it, hero! You've defeated the second mightiest dragon in all of Obsidian!"),
                    ...
                    new NPCDialogue(ALEC_KINCADE, "Thank you again for saving not only our historian, but also RuneRogue as well! We are eternally indebted to you. " +
    ...
    private static void dallasDialogue(Player player, NPC npc) {
        ...
        if (PlayerCounter.DRAGON_SLAYER_2.get(player) == 10) {
            player.dialogue(
                    new PlayerDialogue("Dallas, do you know the location of the mountain that Vorkath is residing? I must defeat him to save Obsidian!"),
    ...
    private static boolean hasReqs (Player player) {
        if (player.getStats().get(StatType.Herblore).fixedLevel >= 73 && PlayerCounter.MONKEY_MADNESS.get(player) >= 3) {
            return true;
        }
        return false;
    }
