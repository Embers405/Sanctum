...
public class DwarfCannon extends OwnedObject {
    ...
    private static final Bounds[] AREA_RESTRICTIONS = {
            ...
            new Bounds(2387, 10355, 2511, 10476, -1),
            new Bounds(2809, 5232, 2970, 5380, -1),
            new Bounds(1192, 1215, 1437, 1342, -1),
            new Bounds(2059, 5632, 2173, 5693, -1),
            new Bounds(1581, 5290, 1819, 5402, -1),
            new Bounds(2240, 4672, 2303, 4735, -1),
            new Bounds(2941, 4726, 3141, 4928, -1),
            new Bounds(1529, 5050, 1613, 5121, -1),
            new Bounds(1529, 5050, 1613, 5121, -1),
            new Bounds(3572, 10163, 3669, 10310, -1),
            new Bounds(2822, 9538, 2875, 9667, -1),
            new Bounds(3082, 3913, 3128, 3954, -1),
            new Bounds(3319, 3905, 3403, 3960, -1),
            new Bounds(3400, 3530, 3455, 3582, -1),
            new Bounds(3397, 9923, 3450, 9976, -1),
            new Bounds(3085, 3486, 3100, 3501, -1),
            new Bounds(1857,5184,1917,5246,-1),
            new Bounds(1984,5183,2048,5248,-1),
            new Bounds(2115,5249,2175,5311,-1),
            new Bounds(2305,5184,2367,5246,-1),
            new Bounds(1426,10069,1454,10089,3),
            new Bounds(2337,9412,2429,9469,0),
    ...
    public void fill() {
/*        if (getOwner().storeAmountSpent > 5) {
            MAX_AMMO = getCannonBallAmmounts(getOwner());
        }*/
    ...
    public void checkDecayTimer() {
        ...
        if (needsDestroyed()) {
            getOwnerOpt().ifPresent(player -> player.sendMessage("<col=ff0000>Your cannon has decayed!</col>"));
            getOwner().cannonLost = true;
            new GroundItem(CANNON_BALL, getAmmo()).owner(getOwner()).spawn();
            setAmmo(0);
            destroy();
        }
    }
    ...
    public boolean handleAreaRestriction() {
        ...
        if (getOwner().tobcannon) {
            getOwner().sendMessage("You can't place a cannon here.");
            return false;
        }
        if (getOwner().wildernessLevel > 56) {
            getOwner().sendMessage("You can't place a cannon above 56 wildy.");
            return false;
        }
        if (getOwner().currentInstance != null) {
        ...
        if (getOwner().getPosition().regionId() == 11593) {
            getOwner().sendMessage("You can't deploy a cannon here.");
            return false;
        }
        return true;
...
/*    public static int getCannonBallAmmounts(Player player) {
