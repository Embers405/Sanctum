...
public class NPC extends NPCAttributes {
    ...
    private final Bounds CATACOMBS_BOUNDS = new Bounds(1600, 9984, 1740, 10110, 0);
    public NPC spawn(int x, int y, int z, Direction spawnDirection, int walkRange) {
        ...
        if (spawnPosition.inBounds(CATACOMBS_BOUNDS)) {
            this.occupyingTiles = false;
            this.getDef().ignoreOccupiedTiles = true;
        }
        if(walkRange > 0)
    ...
    public Point[] getActorTiles() {
        ...
        for (int i = 1; i < getSize() + 1; i++) {
            for (int k = 0; k < Region.SIZES[i].length; k++) {
                int x3 = getX() + Region.SIZES[i][k][0];
                int y3 = getY() + Region.SIZES[i][k][1];
