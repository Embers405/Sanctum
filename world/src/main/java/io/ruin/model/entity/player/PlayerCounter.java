...
public enum PlayerCounter {
    ...
    //Quests
    QUESTS_COMPLETED((p, amt) -> p.completedQuests = amt, p -> p.completedQuests),
    DORICS_QUEST((p, amt) -> p.doricsQuest = amt, p -> p.doricsQuest),
    SHEEP_SHEARER((p, amt) -> p.sheepShearer = amt, p -> p.sheepShearer),
    COOKS_ASSISTANT((p, amt) -> p.cooksAssistant = amt, p -> p.cooksAssistant),
    DRAGON_SLAYER_2((p, amt) -> p.dragonslayerTwo = amt, p -> p.dragonslayerTwo),
    MONKEY_MADNESS((p, amt) -> p.monkeyMadness = amt, p -> p.monkeyMadness),
    SANTA((p, amt) -> p.santaQuest = amt, p -> p.santaQuest),
    ...
    //Reputation
    KING_ARTHUR((p, amt) -> p.kingArthurRep = amt, p -> p.kingArthurRep),
    SIR_KAY((p, amt) -> p.sirKayRep = amt, p -> p.sirKayRep),
    PENTYN((p, amt) -> p.pentynRep = amt, p -> p.pentynRep),
    COUNCILLOR_ANDREWS((p, amt) -> p.councillorAndrews = amt, p -> p.councillorAndrews),
    KING_ROALD((p, amt) -> p.kingRoald = amt, p -> p.kingRoald),
    AUBURY_REP((p, amt) -> p.auburyRep = amt, p -> p.auburyRep),
    THESSALIA_REP((p, amt) -> p.thessaliaRep = amt, p -> p.thessaliaRep),
    ELLAMARIA_REP((p, amt) -> p.ellamariaRep = amt, p -> p.ellamariaRep),
    FLYNN_REP((p, amt) -> p.flynnRep = amt, p -> p.flynnRep),
    DOORMAN_REP((p, amt) -> p.doormanRep = amt, p -> p.doormanRep),
    HERQUIN_REP((p, amt) -> p.herquinRep = amt, p -> p.herquinRep),
    WAYNE_REP((p, amt) -> p.wayneRep = amt, p -> p.wayneRep),
    ...
    CRAFTED_WRATH((p, amt) -> p.craftedWrath = amt, p -> p.craftedWrath),
    ...
    PRIFDDINAS_COURSE((p, amt) -> p.prifddinasLaps = amt, p -> p.prifddinasLaps),
    ...
    CHOPPED_CELASTRUS((p, amt) -> p.choppedCelastrus = amt, p -> p.choppedCelastrus),
    ...
    MINED_TEPHRA((p, amt) -> p.minedTephra = amt, p -> p.minedTephra),
     ...
     * Prayer
     */
    FIENDISH_ASHES((p, amt) -> p.fiendishAshesSpread = amt, p -> p.fiendishAshesSpread),
    VILE_ASHES((p, amt) -> p.vileAshesSpread = amt, p -> p.vileAshesSpread),
    MALICIOUS_ASHES((p, amt) -> p.maliciousAshesSpread = amt, p -> p.maliciousAshesSpread),
    INFERNAL_ASHES((p, amt) -> p.infernalAshesSpread = amt, p -> p.infernalAshesSpread),
    ABYSSAL_ASHES((p, amt) -> p.abyssalAshesSpread = amt, p -> p.abyssalAshesSpread),
    ...
    GROWN_ATTAS((p, amt) -> p.grownAttas = amt, p -> p.grownAttas),
    GROWN_IASOR((p, amt) -> p.grownIasor = amt, p -> p.grownIasor),
    GROWN_KRONOS((p, amt) -> p.grownKronos = amt, p -> p.grownKronos),
    HARVESTED_HESPORI((p, amt) -> p.harvestedHespori = amt, p -> p.harvestedHespori),
    ...
    GROWN_REDWOOD((p, amt) -> p.grownRedWood = amt, p -> p.grownRedWood),
    GROWN_CELASTRUS((p, amt) -> p.grownCelastrus = amt, p -> p.grownCelastrus),
    ...
    TOTAL_LEAGUE_POINTS((p, amt) -> p.totalLeaguePoints = amt, p -> p.totalLeaguePoints),

    TWISTED_LEAGUE_POINTS((p, amt) -> p.twistedLeaguePoints = amt, p -> p.twistedLeaguePoints),

    TRAILBLAZER_LEAGUE_POINTS((p, amt) -> p.trailblazerLeaguePoints = amt, p -> p.trailblazerLeaguePoints),

    SHATTERED_RELICS_LEAGUE_POINTS((p, amt) -> p.shatteredrelicsLeaguePoints = amt, p -> p.shatteredrelicsLeaguePoints)
    ;
