...
public abstract class Entity {
   ...
   public final Event startPersistableEvent(EventConsumer consumer) {
    ...
    public boolean disease(int damage) {
        if(poisonLevel > 0 || isPoisonImmune())
            return false;
        poisonTicks = 120;
        poisonDamage = damage;
        ...
        if(player != null) {
            player.sendMessage("You have been poisoned!");
            Config.DISEASED.set(player, 1);
        }
        return true;
    }

    public void cureDisease(int immuneFor) {
        if(poisonLevel > 1) {
            poisonLevel--;
            poisonDamage = 8;
            Config.DISEASED.set(player, 1);
