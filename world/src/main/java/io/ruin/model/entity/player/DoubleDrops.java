...
//        chance += (gearCount(player) * 2);
    ...
    private static int getDoubleDropChance(Player player) {
        ...
        if (player.isGroups(SecondaryGroup.DRAGON)) {
            doubleDropChance = SecondaryGroup.DRAGON.getDoubleDropChance();
        } else if (player.isGroups(SecondaryGroup.RUNE)) {
            doubleDropChance = SecondaryGroup.RUNE.getDoubleDropChance();
        } else if (player.isGroups(SecondaryGroup.ADAMANT)) {
            doubleDropChance = SecondaryGroup.ADAMANT.getDoubleDropChance();
        } else if(player.isGroups(SecondaryGroup.MITHRIL)) {
            doubleDropChance =SecondaryGroup.MITHRIL.getDoubleDropChance();
        }else if (player.isGroups(SecondaryGroup.GOLD)) {
            doubleDropChance = SecondaryGroup.GOLD.getDoubleDropChance();
        } else if (player.isGroups(SecondaryGroup.IRON)) {
            doubleDropChance = SecondaryGroup.IRON.getDoubleDropChance();
        } else if (player.isGroups(SecondaryGroup.BRONZE)) {
            doubleDropChance = SecondaryGroup.BRONZE.getDoubleDropChance();
        }
