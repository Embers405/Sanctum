...
public class TierThreeDungeon {
    private static final Bounds TierOneSpawns = new Bounds(1673, 4692, 1690, 4710, 0);
    static {
        ObjectAction.register(31618, 1, (player, obj) -> {
            if (player.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC)) {
                enterDZInstance(player);
                return;
            }
            player.dialogue(new OptionsDialogue("Are you sure you're ready to enter(Protect Magic is Advised)",
    ...
    public static void enterDZInstance(Player player) {
        ...
        player.startEvent(e -> {
            ...
            for (int i = 0; i < 4; i++) {
                NPC gwn = new NPC(13192).spawn(map.convertPosition(TierOneSpawns.randomPosition()));
                map.addNpc(gwn);
