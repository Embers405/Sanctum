...
import io.ruin.model.map.`object`.actions.impl.dungeons.SmokeDungeon
...
object LastManStanding {
    ...
    private val LOBBY = Bounds(3138, 3638, 3145, 3645, -1)


    init {
        MapListener.registerBounds(LOBBY).onEnter { player: Player ->
            player.openInterface(
                InterfaceType.PRIMARY_OVERLAY,
                333
            )
        }
            .onExit { player: Player, logout: Boolean ->
        ...
        whenObjClick(39640, 1) { player, obj ->
            if (obj.x == 3142 && obj.y == 3639 && player.absX > 3141 && player.absX > 3142) {
                if (player.lmsQueue != null) {
                    player.event {
                        if (player.options("Leave queue.", "Stay in queue.") == 1) {
                            if (player.lmsQueue != null) {
                                player.lmsQueue.remove(player)
                                player.teleport(3141, 3637, 0)
                            }
                        }
                    }
                } else {
    ...
    fun randomLobbyTile() = Position(1759 + random(8), 3603 + random(2), 0)
