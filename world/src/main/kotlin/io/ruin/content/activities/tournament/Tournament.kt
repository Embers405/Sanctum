...
/*            player.teleportListener = TeleportListener { p: Player ->
                p.sendMessage("The use of these spells is not permitted within the battlegrounds.")
                false
            }*/
...
/*        if (player.tournamentPouch) {
        ...
        if (player.vengeanceActive) {
            ...
            player.packetSender.sendWidget(Widget.VENGEANCE, 0)
        }
        ...
        if (player.cachedRunePouchTypes != 0) {
            ...
            Config.RUNE_POUCH_TYPES.set(player, 0)
            Config.RUNE_POUCH_AMOUNTS.set(player, 0)
        }
    ...
    private fun Player.resolvePrize() {
        when (contestants) {
            3 -> {
                ...
                message(Color.RED.wrap("Congratulations! You have placed third within the tournament. You have won 1.25M GP"))
            }
            2 -> {
                inventory.add(COINS_995, 2_500_000)
                message(Color.RED.wrap("Congratulations! You have placed second within the tournament. You have won 2.5M GP"))
            }
            1 -> {
                inventory.add(COINS_995, 5_000_000)
                message(Color.RED.wrap("Congratulations! You have placed first within the tournament. You have won 5M GP"))
            }
            else -> {
