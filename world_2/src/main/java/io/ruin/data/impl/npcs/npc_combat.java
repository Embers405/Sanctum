...
import io.ruin.data.impl.slayer_tasks;
...
public class npc_combat extends DataFile {
    ...
    public static void main(String[] args) throws Exception {
        Server.fileStore = new FileStore("%HOME%/Desktop/Drako_NPC_DATA/Server/cache/");
        Server.dataFolder = FileUtils.get("%HOME%/Desktop/Drako_NPC_DATA/");
        ItemDef.load();
        NPCDef.load();
        AnimDef.load();
        ...
        if (!Server.dataOnlyMode) {
            list.forEach(info -> {
                for (int id : info.ids) {
                    ...
                    if (def == null) {
                        System.err.println("Fail " + id + " for npc_combat.json");
                        continue;
                    }
...
//                    }
    ...
    public static void dumpAll() {
        ...
        try(BufferedWriter bw = new BufferedWriter(new FileWriter(FileUtils.get("%HOME%/Desktop/Drako_NPC_DATA/Server/data/wiki_dumps/combat/temp/~FAILED.txt")))) {
    ...
    public static final class WikiDumper {
        ...
        public WikiDumper run() {
            File file = null;
            try {
                Connection.Response response = Jsoup.connect("https://oldschool.runescape.wiki/w/" + URLEncoder.encode(wikiName, "UTF-8"))
                ...
                try(BufferedWriter bw = new BufferedWriter(new FileWriter(file = FileUtils.get("%HOME%/Desktop/Drako_NPC_DATA/Server/data/wiki_dumps/combat/temp/" + wikiName + ".txt")))) {
                    int count = 0;
    ...
    private static void toJson() throws IOException {
        File temp = FileUtils.get("%HOME%/Desktop/Drako_NPC_DATA/Server/data/wiki_dumps/combat/temp");
        if(!temp.exists())
    ...
    private static void toJson(File dumpFile) throws IOException {
        ...
        try(BufferedWriter bw = new BufferedWriter(new FileWriter(FileUtils.get("%HOME%/Desktop/Revenant_NPC_DATA/Server/data/wiki_dumps/combat/temp/" + dumpFile.getName().replace(".txt", ".json"))))) {
