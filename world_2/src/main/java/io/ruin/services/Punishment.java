...
public class Punishment {
    ...
    public static void jail(Player p1, Player p2, long time) {
        ...
        System.out.println(TimeUtils.fromMs(time - System.currentTimeMillis(), true));
        p1.sendMessage("Jailing " + p2.getName() + " for " + TimeUtils.fromMs(time - System.currentTimeMillis(), false));
        ...
        logPunishment(p1, p2, -2, "jailed", new MessageEmbed.Field("time", TimeUtils.fromMs(time, true), true));
    }

    public static void jail(Player player, NPC npc, long time) {
        if (player.lmsSession != null) {
            player.lmsSession.eliminatePlayer(player);
        }
        player.jailerName = npc.getDef().name;
        player.jailTime = time;
        World.startEvent(e -> {
            if (!player.isOnline())
                return;
    ...
    public static void unjail(Player p1, Player p2) {
        p2.jailTime = 0; //this will automatically finish their sentence lol
