...
//            return;
    ...
    private static final TickDelay dropdelay = new TickDelay();
    private static TickDelay pickupdelay = new TickDelay();
    ...
    public static void logDropTrade(int takerId, int dropperId, String takerIp, String dropperIp, String takerName, String dropperName, int itemId, int amount, int x, int y, int z, long timeDropped) {
        ...
        if (value >= 5_000_000) {
            for(Player p : World.players) {
                if (p.getName().equalsIgnoreCase(takerName)) {
                    notifyStaffPickup(p, value);
                }
                if (p.getName().equalsIgnoreCase(dropperName)) {
                    notifyStaffDrop(p, value);
                }
            }
        }
