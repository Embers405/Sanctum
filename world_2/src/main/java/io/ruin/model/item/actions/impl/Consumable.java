...
public class Consumable {
    ...
    static {
        ...
        registerPotion(Potion.DIVINE_MAGIC, p ->{ p.getStats().get(StatType.Magic).boost(5, 0.15);
            p.getStats().get(StatType.Defence).boost(5, 0.15);
    });
    ...
    private static boolean drink(Player player, Potion potion, Item item, int newId) {
        ...
        if (player.divineMagicBoostActive && potion == Potion.DIVINE_RANGING) {
            player.sendMessage("Your magic boost is still active.");
            return false;
        }
        if (player.divineAttackBoostActive && potion == Potion.DIVINE_RANGING) {
            player.sendMessage("Your attack boost is still active.");
            return false;
        }
        if (player.divineStrBoostActive && potion == Potion.DIVINE_RANGING) {
            player.sendMessage("Your strength boost is still active.");
            return false;
        }
        if (player.divineDefBoostActive && potion == Potion.DIVINE_RANGING) {
            player.sendMessage("Your defence boost is still active.");
            return false;
        }
        if (player.divineSuperCmbBoostActive && potion == Potion.DIVINE_RANGING) {
