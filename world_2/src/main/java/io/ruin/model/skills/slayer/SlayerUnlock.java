...
public enum SlayerUnlock {
    ...
    ADAMIND_SOME_MORE(Config.ADA_MIND_SOME_MORE, 100, 40, true),
    ...
    static {
        ...
        InterfaceHandler.register(426, handler ->  {
            ...
            handler.actions[20] = (SimpleAction) SlayerUnlock::extendAll;
    ...
    private static void cancelTask(Player player) {
        ...
        if (Config.SLAYER_POINTS.get(player) < pointsCost(player)) {
            player.sendMessage("You don't have enough slayer points to cancel your task.");
            return;
        }
        Config.SLAYER_POINTS.set(player, Config.SLAYER_POINTS.get(player) - pointsCost(player));
        Slayer.sendRewardInfo(player);
        Slayer.reset(player);
        player.sendMessage("Your slayer task has been cleared. You may now receive another one.");
    }

    private static int pointsCost(Player player) {
    ...
    public static void openRewards(Player player) {
        ...
        player.getPacketSender().sendAccessMask(426, 23, 0, 8, 1052);
    }
    ...
    private enum ShopItem {
        ...
        NIEVE_JR_PET(30140, 5000, 1), //Nieve Jr pet
        ECUMENICAL_KEY(11942, 15, 1), //Ecumenical Keys
        SHAYZIEN_SET(13569, 200, 1), //Shayzien Set
        CRYSTAL_WEAPON_SEED(4207, 1200, 1), //Crystal Weapon seed
        ;
