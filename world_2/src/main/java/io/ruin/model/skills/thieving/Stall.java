...
public enum Stall {
    ...
    CRAFTING_STALL(5, 7, 25.0, 49000, "crafting stall",
    ...
    SEED_STALL(27, 3, 75.0, 40000, "seed stall",
    ...
    private static void attempt(Player player, Stall stall, GameObject object, int replacementID) {
        ...
        player.startEvent(event -> {
            ...
            {
                if(player.isDonator()) {
                    loot = new Item(995, 500 * stall.levelReq);
                }else{
                    loot = stall.lootTable.rollItem();
                }
            }else{
                loot = stall.lootTable.rollItem();
            }
            ...
            switch (stall) {
                ...
                case FISH_STALL:
                    ...
                    break;
                case TEA_STALL:
                    player.getDiaryManager().getVarrockDiary().progress(VarrockDiaryEntry.THESSALIA_SHOP);
                    break;
                case SILVER_STALL:
                    if (player.getPosition().regionId() == 6968) {
                        player.getDiaryManager().getKourendDiary().progress(KourendDiaryEntry.STEAL_SILVER);
                    }
                    break;
            }
        });
