...
public class Hunter {
    ...
    public static int getMaxTraps(Player player) {
        if(player.wildernessLevel > 0 && player.getStats().get(StatType.Hunter).currentLevel >= 80) {
            count = 1;
        }
        int level = Math.min(99, player.getStats().get(StatType.Hunter).currentLevel);
        return 1 + (level / 20) + count;
    }
    ...
    public static void registerTrap(TrapType type, boolean registerItemActions) {
        if (registerItemActions) {
            ...
            GroundItemAction.register(type.getItemId(), "lay", (player, groundItem, distance) -> placeGroundItemTrap(player, type, groundItem));
        }
