...
public class CaveKraken extends NPCCombat {
    ...
    public boolean attack() {
        if (!withinDistance(10) && npc.getId() != 5535) {
            return false;
        }
        ...
        int damage = info.max_damage;
        int delay = getProjectile().send(npc, target);
        Hit hit = new Hit(npc, info.attack_style)
                .randDamage(target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC) ? 0 : damage)
                .clientDelay(delay);
        hit.postDamage(t -> {
            if(hit.damage > 0) {
    ...
    public Position getDropPosition() {
        if (getKiller() == null) return npc.getPosition();
