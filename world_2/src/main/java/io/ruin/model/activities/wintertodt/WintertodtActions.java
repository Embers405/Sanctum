...
public class WintertodtActions {
    ...
    private static void feed(Player player, Brazier b) {
        if(World.doubleWintertodt) pointmod = 2;
        ...
        player.startEvent(event -> {
            while (true) {
                ...
                if (player.getInventory().remove(Wintertodt.BRUMA_KINDLING, 1) > 0) {
                    ...
                    Wintertodt.addPoints(player, 50*pointmod);
                } else {
                    ...
                    Wintertodt.addPoints(player, 20*pointmod);
                }
                event.delay(2);
            }
        });
    ...
    private static void light(Player player, Brazier b) {
        if(World.doubleWintertodt) pointmod = 2;
        if (player.getInventory().contains(Tool.TINDER_BOX, 1) || player.getEquipment().hasId(20720) || player.getInventory().contains(20720)) {
    ...
    private static void healPyromancer(Player player, Item item, NPC npc) {
        ...
        Wintertodt.addPoints(player, 60*pointmod);
    }
