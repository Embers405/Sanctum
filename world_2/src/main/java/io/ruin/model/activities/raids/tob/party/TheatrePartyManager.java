...
//        }
    ...
    public void deregister(TheatreParty party) {
        ...
        if (parties.contains(party)) {
            Player player = World.getPlayer(party.getLeaderId());
            if (player != null) {
                player.deathEndListener = null;
                player.sendMessage("Disbandoned your party");
            }
            parties.remove(party);
        } else {
            World.getPlayer(party.getLeaderId()).sendMessage("Disbandoned your party");
        }
    }
     ...
     * @param player
     */
    public void openPartyDetailsViaSlot(Player player, int slot) {
        getPartyForSlot(slot).ifPresent((party) -> {
    ...
    public void openPartyDetails(Player player) {
        System.out.println(getPartyForPlayer(player.getUserId()).get().getLeader().get().getName());
    ...
    public Optional<TheatreParty> getPartyForPlayer(int userId) {
        return parties.stream().filter(p -> (p.isLeader(userId) || p.inParty(userId)) && forUserId(userId).isPresent()).findFirst();
    }
    ...
    static {
        ...
        ObjectAction.register(32653, "enter", (player, obj) -> {
           // player.sendMessage("Theatre of blood is disabled currently.");
    ...
    private static void crystals(Player player, TheatreParty party) {
        if (!player.isAcceptedTheatreCrystals()) {
            player.dialogue(new OptionsDialogue(Color.MAROON.wrap("Only Verzik's crystals can teleport out of the Theatre."),
                    new Option("Enter the Theatre of Blood", () -> enterTheatre(player, party))
            ));
        } else {
            ...
            player.dialogue(new OptionsDialogue("Is your party ready? (Members: " + party.getUsers().size() + ")",
                    new Option("Yes, let's go!", () -> {
                        party.forPlayers(p -> {
                            ...
                            p.getInventory().addOrDrop(25961, 1);
                            p.theatreroom = "";
                        });
