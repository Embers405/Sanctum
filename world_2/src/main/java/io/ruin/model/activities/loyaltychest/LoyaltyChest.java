...
public class LoyaltyChest {
    ...
    static {
        ...
        ObjectAction.register(LOYALTY_CHEST, "open", (player, obj) -> openChest(player));
        ...
        InterfaceHandler.register(Interface.LOYALTY_CHEST, h -> {
            h.actions[19] = (SimpleAction) p -> claimReward(p, 1);
            h.actions[22] = (SimpleAction) p -> claimReward(p, 2);
            h.actions[25] = (SimpleAction) p -> claimReward(p, 3);
            h.actions[28] = (SimpleAction) p -> claimReward(p, 4);
            h.actions[31] = (SimpleAction) p -> claimReward(p, 5);
            h.actions[34] = (SimpleAction) p -> claimReward(p, 6);
            h.actions[37] = (SimpleAction) p -> claimReward(p, 7);
            h.actions[40] = (SimpleAction) p -> claimReward(p, 8);
            h.actions[43] = (SimpleAction) p -> claimReward(p, 9);
            h.actions[46] = (SimpleAction) p -> claimReward(p, 10);
        });
