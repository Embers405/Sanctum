...
public class ChambersOfXeric {
    ...
    public static void createChamber(Player player, Party party) {
        ...
        for (Player other : party.getMembers()) {
            if (player.getIpInt() == other.getIpInt()) {
                count++;
                if (count > 2) {
                    player.sendMessage(Color.DARK_RED.wrap("You can only have 2 accounts inside Chambers of Xeric!"));
                    return;
                }
            }
        }
    ...
    private void exited(Player player, boolean logout) {
        ...
        TabJournal.swap(player,Interface.QUEST_TAB);
    }
    ...
    public void startRaid() {
        ...
        combatPointsFactor = 1.5 * party.getSize();
    }
