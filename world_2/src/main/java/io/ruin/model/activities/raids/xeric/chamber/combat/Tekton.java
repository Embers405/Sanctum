...
public class Tekton extends NPCCombat {
    ...
    public void init() {
        ...
        npc.addEvent(event -> {
            ...
            npc.faceNone(false);
            npc.unlock();
            if(npc.getId() == 7541 || npc.getId() == 7542 || npc.getId() == 7543 || npc.getId() == 7544 || npc.getId() == 7545) {
                double factor = 1.1 + (0.20 * p.raidsParty.getSize());
                npc.getCombat().getStat(StatType.Hitpoints).fixedLevel *= factor;
            ...
            for (Player player : npc.localPlayers()) {
                ...
                if ((side.deltaX != 0 && player.getAbsX() - src.getX() == side.deltaX) ||
                    ...
                    if (player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE))
                        maxDamage = 20;
                    player.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(10, maxDamage).ignorePrayer().delay(0));
                }
            }
        });
        return true;
    ...
    private void smith() {
        npc.addEvent(event -> {
            ...
            for (int i = 0; i < 6; i++) {
                shootLava();
                event.delay(3);
                npc.incrementHp(Math.max((int) (1200 * 0.005), 1));
            }
            npc.animate(7474);
            event.delay(2);
            npc.transform(7541);
