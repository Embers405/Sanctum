...
public class GodwarsFollower {
    ...
    private static final int[] ZAMORAK_FOLLOWER = {3130, 3131, 3132, 3129, 3159, 3133, 3141, 3138, 484, 485, 486, 487, 3134, 5007,
            3135, 3136, 3137, 3140, 3139, 3161, 3160, 6495};
    ...
    private static final int[] ANCIENT_FOLLOWER = {11293, 11294, 11291, 11292, 11290};

    private static Bounds GODWARS = new Bounds(2817, 5183, 2941, 5373, -1);
    ...
    private static final Predicate<ItemDef> ZAROS_ITEM = def -> def.name.toLowerCase().contains("ancient") || def.name.toLowerCase().contains("zaryte") || def.name.toLowerCase().contains("torva") || def.name.toLowerCase().equals("book of darkness") || def.name.toLowerCase().equals("ancient blessing");
    ...
    static {
        ...
        ItemDef.cached.values().forEach(def -> {
            ...
            if (ZAROS_ITEM.test(def)) def.godItem[3] = true;
        });
...
//            }
         ...
         * Ancient
         */
        SpawnListener.register(ANCIENT_FOLLOWER, npc -> {
            if (npc.getPosition().inBounds(GODWARS)) {
//                npc.deathEndListener = (DeathListener.SimplePlayer) killer -> Config.GWD_ZAROS_KC.set(killer.player, Config.GWD_ZAROS_KC.get(killer.player) + 1);
//                if (npc.aggressionImmunity == null)
//                    npc.aggressionImmunity = isProtected(2);
            }
        });
