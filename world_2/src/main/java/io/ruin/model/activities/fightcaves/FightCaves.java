    ...
    private void stop(boolean logout) {
        if(logout) {
            ...
            if(lastWave == 0) {
                ...
                if(!killedJad) {
                    if (getStartingWave(player) == lastWave) {
                        tokkul = 0;
                    }
                    if(!player.isBronze()) {
                        if (tokkul > 0) {
                            ...
                            player.dialogue(new NPCDialogue(HOST, "Well I suppose you tried... but you didn't earn any TokKul this time.").animate(588));
                        }
                    } else {
                        player.dialogue(new NPCDialogue(HOST, "Well I suppose you tried... better luck next time."));
                    }
                } else {
                    player.dialogue(new NPCDialogue(HOST, "You have defeated TzTok-Jad, I am most impressed! Please accept this gift. Give the cape back to me if you do not want it.").animate(615));
