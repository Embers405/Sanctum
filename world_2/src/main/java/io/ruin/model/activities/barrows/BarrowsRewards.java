...
public class BarrowsRewards {
    ...
    public static int[] pieces = {4708, 4710, 4712, 4714, 4716, 4718, 4720, 4722, 4724, 4726, 4728, 4730, 4732, 4734, 4736, 4738, 4745, 4747, 4749, 4751, 4753, 4755, 4757, 4759};
    ...
    public static ItemContainer loot(Player player) {
        ...
        for(int i = 0; i < brothersKilled; i++) {
            if(Random.get() > customBarrowsChance) {
                ...
                Broadcast.WORLD.sendNews(player, player.getName() + " just received " + def.descriptiveName + " from the Barrows chest!");
                container.add(def.id, 1);
            }
        }
        if (player.getEquipment().hasAtLeastOneOf(pieces)) {
