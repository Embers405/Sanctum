...
public class VasaNistirio extends NPCCombat {
    ...
    public void init() {
        ...
        closeTeleportPositions = npc.getSpawnPosition().relative(1, 1).area(2, pos -> !pos.inBounds(npc.getBounds())); // edges of the boss
        farTeleportPositions = new ArrayList<>(36);
        farTeleportPositions.addAll(npc.getSpawnPosition().relative(1, 1).area(2));
        farTeleportPositions.addAll(npc.getSpawnPosition().relative(1, 1).area(2));
        farTeleportPositions.addAll(npc.getSpawnPosition().relative(1, 1).area(2));
        farTeleportPositions.addAll(npc.getSpawnPosition().relative(1, 1).area(2));
    }
    ...
    private void rocks() {
        npc.localPlayers().forEach(p -> {
            if (p.getCombat().isDead() || Misc.getEffectiveDistance(npc, p) >= 10 || !ProjectileRoute.allow(npc, p))
                return;
            int delay = ROCK_PROJECTILE.send(npc, p.getLastPosition().copy());
            Position targetPos = p.getLastPosition().copy();
            World.sendGraphics(305, 35, delay, targetPos);
            p.addEvent(event -> {
                if (!p.getPosition().isWithinDistance(targetPos, 1)) {
                    ...
                    minDamage /= 2;
                    maxDamage /= 2;
                    target.hit(new Hit(npc, AttackStyle.RANGED, null).randDamage(minDamage,maxDamage).ignoreDefence().ignorePrayer());
                }
            });
        });
    }
