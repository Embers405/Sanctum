...
public class DailyTask {
    ...
    public enum PossibleTasksHard {
        ...
        private PossibleTasksHard(DailyTaskDifficulty type, int amount, String message) {
            this.type = type;
            this.amount = amount;
    ...
    public enum PossibleTasksMedium {
        ...
        private PossibleTasksMedium(DailyTaskDifficulty type, int amount, String message) {
            this.type = type;
            this.amount = amount;
            this.message = message;
        }
    ...
    public enum PossibleTasksEasy {
        ...
        private PossibleTasksEasy(DailyTaskDifficulty type, int amount, String message) {
    ...
    public static void assignTask(Player player) {
        if (player.dailyTaskDate == getTodayDate()) {
            if (player.dailyTaskCompletePoints == 3) {
                player.sendMessage("<col=0000FF>You have already completed your daily tasks.");
            }
            if (player.currentTaskEasy != null || player.currentTaskMedium != null || player.currentTaskHard != null) {
                player.sendMessage("<col=0000FF>You already have daily tasks. You can check your current tasks from your quest tab!");
            }
        }
        if (player.dailyTaskDate != getTodayDate()) {
    ...
    public static void open(Player c) {
        if (c.currentTaskEasy == null) {
            c.sendScroll("<col=8B0000>Daily Task Manager",
                    "Tasks reset at 12:00am server time",
            ...
            c.sendScroll("<col=8B0000>Daily Task Manager",
                    "Tasks reset at 12:00am server time",
                    "<col=8B0000>---------------- Easy Task ----------------",
                    "<col=0000FF>"+c.currentTaskEasy.message+"",
                    "<col=0000FF>Progress: " + c.totalDailyDone + "/" + c.currentTaskEasy.amount + "",
                    "",
                    "<col=8B0000>---------------- Medium Task ----------------",
                    "<col=0000FF>"+c.currentTaskMedium.message+"",
                    "<col=0000FF>Progress: " + c.totalDailyMediumDone + "/" + c.currentTaskMedium.amount + "",
                    "",
                    "<col=8B0000>---------------- Hard Task ----------------",
                    "<col=0000FF>"+c.currentTaskHard.message+"",
                    "<col=0000FF>Progress: " + c.totalDailyHardDone + "/" + c.currentTaskHard.amount + ""
            );
        }
    }
