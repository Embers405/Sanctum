...
public class StaffModerationInterface {
    ...
    public static void setActionModerate(Player player) {
        ...
        player.sendMessage("right click success mpanel");
    }
            ...
            case 26://confirm
                ...
                switch (getActionSelected()) {
                    ...
                    case 49://jail (integer input for time jail)
                        for (Player p2: World.players)
                            if (p2.getName().equalsIgnoreCase(getPlayerSelected())) {
                                player.integerInput("How long would you like to jail the user for? (In Minutes)", time -> {
                                        Punishment.jail(player, p2, System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(time));
                                });
                            }
                        break;
                    case 51://unmute
