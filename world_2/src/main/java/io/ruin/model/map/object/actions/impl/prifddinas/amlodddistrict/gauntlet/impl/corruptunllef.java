...
import io.ruin.model.activities.barrows.linza.LinzaTheDisgraced;
...
public class corruptunllef extends NPCCombat {
    ...
    private void preDefend(Hit hit) {
        if (npc.getId() == 9035 && hit.attackStyle.isMelee()) {
            hit.block();
        } else if (npc.getId() == 9036 && hit.attackStyle.isMagic()) {
            hit.block();
        } else if (npc.getId() == 9037 && hit.attackStyle.isCannon() || npc.getId() == 9037 && hit.attackStyle.isRanged()) {
            hit.block();
        }
    }
    ...
    private void postDefend(Hit hit) {
        if (Random.rollDie(10, 1)) {
            npc.transform(Random.get(9035,9036,9037));
        }
    }
