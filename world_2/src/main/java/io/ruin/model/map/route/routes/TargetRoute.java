...
public class TargetRoute {
    ...
    private final int[] PestPortal = {1747, 1748, 1749, 1750};
    private void afterMovement0(Entity entity) {
/*        for (int portal : PestPortal) {
            if (target.npc.getId() == portal) {
                finishAction.run();
            }
        }*/
        if(finishAction != null) {
            ...
            if(withinDistance || (target.npc != null && target.npc.skipReachCheck != null && target.npc.skipReachCheck.test(entity.getPosition())) ||
                    (EntityInteractionMode.isAPInteraction(target.npc) && ProjectileRoute.allow(entity, target))) {
                finishAction.run();
