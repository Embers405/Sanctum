...
public class NPCMovement extends Movement {
    ...
    private void randomWalk() {
        if(!npc.isRandomWalkAllowed())
            return;
        if(npc.getDef().name.equalsIgnoreCase("fishing spot"))
            return;
        if(npc.getDef().walkAnimation == npc.getDef().standAnimation)
            return;
        if(npc.getDef().walkAnimation == -1)
            return;
        if (!npc.getMovement().isAtDestination())
            return;
        if(!Random.rollDie(1000, 260))
            return;
