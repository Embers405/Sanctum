...
public class MakeoverMage {
    ...
    private static void open(Player player, NPC npc) {
        if(!player.getEquipment().isEmpty()) {
            player.dialogue(new NPCDialogue(npc, "Please remove your equipment before we proceed with the make-over."));
            return;
        }
    ...
    static {
        NPCAction.register(1307, "Talk-to", MakeoverMage::titles);
        NPCAction.register(1307, "Makeover", MakeoverMage::open);
    }
