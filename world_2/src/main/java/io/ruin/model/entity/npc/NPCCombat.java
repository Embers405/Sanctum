...
import static io.ruin.model.World.npcs;
...
import static io.ruin.model.map.route.routes.DumbRoute.step;
...
//        Wilderness.resourcePackWithBoss(pKiller, npc);
    ...
    private void handleDrop(Killer killer, Position dropPosition, Player pKiller, List<Item> items) {
        for(Item item : items) {
            ...
            if (item.getId() == 534 || item.getId() == 536 || item.getId() == 6812 || item.getId() == 11943 || item.getId() == 22124) {
                if (pKiller.isBronze() && pKiller.wildernessLevel > 0) {
            ...
            if (item.getDef().name.toLowerCase().contains("grimy")) {
                if (pKiller.isAdamant() && pKiller.wildernessLevel > 0) {
    ...
    private void getRareDropAnnounce(Player pKiller, Item item) {
        ...
        if (item.lootBroadcast != null) {
            ...
            Broadcast.WORLD.sendNews(pKiller, message + " from " + npc.getDef().descriptiveName + "!");
        }
    ...
    public void DiaryKCCheck(Player player, NPC npc) {
        ...
        if (npc.getDef().name.equalsIgnoreCase("Avatar of Creation")) {
            player.getDiaryManager().getKourendDiary().progress(KourendDiaryEntry.KILL_VBOSS);
        }
    }


    public static NPC getNpc(int npcId) {
        for (NPC npc : npcs) if (npc != null && npc.getId() == npcId) return npc;
        return null;
    }
    ...
    public static void attackStyleChange(NPC npc, Entity target) {

    }

    public static void handleFollowing(NPC npc, Entity target) {
        step(npc, target, 10);
    }
}