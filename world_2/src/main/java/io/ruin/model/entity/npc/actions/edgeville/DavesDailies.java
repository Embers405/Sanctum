...
public class DavesDailies {
    static {
        NPCAction.register(3984, "talk-to", (player, npc) -> {
                            ...
                            new Option("What are daily tasks?", () -> {
                                player.dialogue(
                                        new NPCDialogue(npc, "Daily tasks are assignments given to you each day to complete within 24 hours. Finishing all three of your" +
                                                "tasks awards you with a FREE Champion Box. Tasks reset every day at 12am server time.")
                                );
                            }),
        ...
        NPCAction.register(3984, "view-tasks", (player, npc) -> {
            DailyTask.open(player);
        });
        NPCAction.register(3984, "open-shop", (player, npc) -> {
            //TODO SETUP A SHOP
        });
    }
    ...
    public static void finishedTasks(Player player, NPC npc) {
        ...
        if (player.claimedDailyCache) {
            player.dialogue(
                    new MessageDialogue("You have already claimed todays reward.")
            );
        }
        if (player.totalDailyDone < 3) {
            player.dialogue(
                    new MessageDialogue("You have not yet finished all three daily tasks.")
            );
        }
        if (player.totalDailyDone == 3 && player.claimedDailyCache == false) {
            int finalAmount = amount;
            player.dialogue(
                    new NPCDialogue(npc, "Ah, yes, you have completed all three tasks assigned to you for today! Please accept this Daily Task Cache as a reward," +
                            " as well as 5 extra daily task points!"),
                    new MessageDialogue("You receive a daily task cache and 5 daily task points!"),
                    new ActionDialogue(() -> {
                        player.claimedDailyCache = true;
