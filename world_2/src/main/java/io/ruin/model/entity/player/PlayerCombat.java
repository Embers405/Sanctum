...
import static io.ruin.cache.ItemID.CRYSTAL_BOW;
...
public class PlayerCombat extends Combat {
    ...
    public void preAttack() {
        ...
        if (target != null) {
            if (!canAttack(target, true))
                ...
                TargetRoute.set(player, target, useSpell() || (wearsNightmareStaff(player) && specialActive != null) ? 10 :
        ...
        try{
            bufWriter =
                    Files.newBufferedWriter(
                            Paths.get("C:/Users/Administrator/Desktop/Obsidian/fucked/fucked.txt"),
    ...
    public void attack() {
        ...
        if (wearsNightmareStaff(player) && specialActive != null) {
    ...
    private void attackWithMagicWeapon() {
        ...
        if (weaponDef.id == HolySanguinestiStaff.CHARGED) {
            ...
            player.animate(1167);
            player.graphics(1900, 92, 0);
            int duration = new Projectile(1899, 30, 0, 51, 56, 10, 16, 11).send(player, target);
            Hit hit = new Hit(player, AttackStyle.MAGIC, getAttackType()).randDamage(32).clientDelay(duration).setAttackWeapon(weaponDef);
            int damage = target.hit(hit);
            if (damage > 0) {
        ...
        if (weaponDef.id == ThammaronsSceptre.CHARGED) {
            if (target.player != null) {
                player.sendMessage(Color.RED.wrap("This staff's spell cannot be used against other players."));
                return;
            }
            if (player.wildernessLevel <= 0) {
                player.sendMessage(Color.RED.wrap("This staff's spell cannot be used outside of the wilderness."));
                return;
            }
            player.animate(1167);
            player.graphics(1540, 92, 0);
    ...
    private void attackWithInfernalScythe(Player player, Entity target) {
        AttackStyle style = attackSet.style;
        AttackType type = attackSet.type;
        int maxDamage = CombatUtils.getMaxDamage(player, style, type);
        updateLastAttack(weaponType.attackTicks);
        if (handleSpecial(style, type, maxDamage))
            return;
        attackAnim();
        final int secondHitDmg = (int) (maxDamage * 0.9f);
        final int thirdHitDmg = (int) (maxDamage * 0.7f);
        final int forthHitDmg = (int) (maxDamage * 0.5f);
        final int fifthHitDmg = (int) (maxDamage * 0.3f);
        final int sixthHitDmg = (int) (maxDamage * 0.1f);
        final Hit firstHit = new Hit(player, style, type).randDamage(maxDamage).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit secondHit = new Hit(player, style, type).randDamage(secondHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit thirdHit = new Hit(player, style, type).randDamage(thirdHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit forthHit = new Hit(player, style, type).randDamage(forthHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit fifthHit = new Hit(player, style, type).randDamage(fifthHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit sixthHit = new Hit(player, style, type).randDamage(sixthHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        player.graphics(1898, 100, 20);
        if (target.getSize() >= 2  || target.npc.getId() == 2668 || target.npc.getId() == 7413) {
            target.hit(firstHit, secondHit, thirdHit,forthHit,fifthHit,sixthHit);
        } else {
            target.hit(firstHit);
        }
    }
    ...
    private void attackWithDrakoWhip(Player player, Entity target) {
        ...
        final Hit secondHit = new Hit(player, style, type).randDamage(secondHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit thirdHit = new Hit(player, style, type).randDamage(thirdHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        if (target.isNpc()  || target.npc.getId() == 2668 || target.npc.getId() == 7413) {
            target.hit(firstHit, secondHit, thirdHit);
        } else {
            target.hit(firstHit);
        }
    ...
    private void attackWitInfernalWhip(Player player, Entity target) {
        AttackStyle style = attackSet.style;
        AttackType type = attackSet.type;
        int maxDamage = CombatUtils.getMaxDamage(player, style, type);
        updateLastAttack(weaponType.attackTicks);
        ...
        final int secondHitDmg = (int) (maxDamage * 0.5f);
        final int thirdHitDmg = (int) (maxDamage * 0.25f);
        final int fifthHitDmg = (int) (maxDamage * 0.30f);
        final Hit firstHit = new Hit(player, style, type).randDamage(maxDamage).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit secondHit = new Hit(player, style, type).randDamage(secondHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        final Hit thirdHit = new Hit(player, style, type).randDamage(thirdHitDmg).setAttackWeapon(player.getEquipment().getDef(Equipment.SLOT_WEAPON));
        if (target.isNpc()  || target.npc.getId() == 2668 || target.npc.getId() == 7413) {
    ...
    private void attackWithMelee() {
        ...
        if (weapon == null && weaponType == WeaponType.UNARMED) {
            ...
            boolean attackWitInfernalWhip = weapon.id == 30374;
            if (hasMaxWhip && target.isNpc()) {
                attackWithDrakoWhip(player, target);
                return;
            }
            ...
            boolean hasInfernalScytheEquipped = weapon.id == 30383;
            if (hasInfernalScytheEquipped && target.isNpc()) {
                attackWithInfernalScythe(player, target);
                return;
            }
    ...
    private void attackWithRanged() {
        ...
        if(rangedWep == RangedWeapon.CHARGED_MAGMA) {
            attackWithBlowpipe(wep);
            return;
        }
        ...
        attackAnim();
        if(!fireDouble) {
            boolean chins = wep.getId() == 10033 || wep.getId() == 10034 || wep.getId() == 11959 || wep.getId() == 23907;
    ...
    private boolean hasAvaAssembler() {
        int capeID = player.getEquipment().getId(Equipment.SLOT_CAPE);
        return capeID == 21898 || capeID == 22109;
    }
    ...
    private void postDefend(Hit hit) {
        ...
        if(hit.attacker != null) {
            ...
            if(!hit.attacker.isPlayer()) {
                if (player.getEquipment().getId(Equipment.SLOT_SHIELD) == 30238 && player.wildernessLevel == 0) {
                ...
                } else  if (player.getEquipment().getId(Equipment.SLOT_SHIELD) == 30238 && Random.rollPercent(70) && player.wildernessLevel >= 1) {
                    hit.damage *= 0.75;
                    player.graphics(5024);
                }
            }
    ...
    private void postDamage(Hit hit) {
        ...
        NightmareZoneDream.check(player, hit);
    ...
    private boolean salveAmuletBoost(Player player, Hit hit, Entity entity) {
        ...
        final int salveAmulete = 10588;
        final int salveAmuletei = 25250;
        final int salveAmuleteii = 12018;
        ...
        if (hit.attackStyle != null && target != null && target.isNpc()) {
            ...
            if (hit.attackStyle.isMelee() && amulet.getId() == salveAmulet) {
                ...
                hit.boostDamage(0.15);
                return true;
            } else if (amulet.getId() == salveAmuletImbuede) {
                hit.boostAttack(0.15);
                hit.boostDamage(0.15);
                return true;
            } else if (amulet.getId() == salveAmulete) {
                hit.boostAttack(0.15);
                hit.boostDamage(0.15);
                return true;
            } else if (amulet.getId() == salveAmuletei) {
                hit.boostAttack(0.15);
                hit.boostDamage(0.15);
                return true;
            } else if (amulet.getId() == salveAmuleteii) {
                hit.boostAttack(0.15);
                hit.boostDamage(0.15);
                return true;
            }
    ...
    private void preTargetDefend(Hit hit, Entity target) {
        ...
        if (hit.attackStyle != null && hit.attackStyle.isMelee() && target.npc != null && player.getEquipment().wearsFireObsidianSet()) {
            hit.boostAttack(0.35); //35% accuracy increase
            hit.boostDamage(0.35); //35% damage increase
        }

        if (hit.attackStyle != null && hit.attackStyle.isMelee() && target.npc != null && player.getEquipment().wearsObsidianSet()) {
            hit.boostAttack(0.75); //75% accuracy increase
            hit.boostDamage(0.75); //75% damage increase
        }


        if(target.npc != null && target.npc.getDef().dragon && hit.attackStyle != null || target.npc != null && target.npc.getId() == 9047 && hit.attackStyle != null) {
        ...
        if(player.getEquipment().hasId(CRYSTAL_BOW)
                 ...
                 * Swords
                 */
                || player.getEquipment().hasId(23995)
                || player.getEquipment().hasId(25872)
                || player.getEquipment().hasId(25874)
                || player.getEquipment().hasId(25876)
                || player.getEquipment().hasId(25878)
                || player.getEquipment().hasId(25880)
                || player.getEquipment().hasId(25882)
                || player.getEquipment().hasId(24551)
        ...
        if(hit.attackStyle != null && hit.attackStyle.isRanged() && (player.getEquipment().hasId(30289))) {
            double strengthLevel = (target.getCombat().getLevel(StatType.Attack) + target.getCombat().getLevel(StatType.Strength) + target.getCombat().getLevel(StatType.Defence) / 3);
            double accuracy = 140D + ((3 * strengthLevel - 10D) / 100D) - ((Math.pow(3 * strengthLevel / 10D - 100D, 2) / 100D));
            accuracy = Math.min(140, accuracy);
    ...
    private void postTargetDamage(Hit hit, Entity target) {
        ...
        /* SOTRR Cripple & Damage */
        if (hit.attackStyle != null && hit.attackStyle.isRanged() && player.SOTRR && player.getInventory().hasItem(26071, 1)) {
            if (Random.rollPercent(10)) {
                player.animate(9158);
                player.graphics(1981, 0, 0);
                target.root(6, true);
                target.addEvent(event -> {
                    for (int i = 0; i < 5; i++) {
                        target.hit(new Hit(player, null, null).fixedDamage(1).ignoreDefence().ignorePrayer());
                        event.delay(1);
                    }
                });
                target.player.getMovement().drainEnergy(30);
            }
        }

        /* 	Sigil of Consistency */
        if (hit.attackStyle != null && hit.attackStyle.isRanged() && hit.attackStyle.isMagic() && hit.attackStyle.isMelee() && player.SOFC && player.getInventory().hasItem(25993, 1)) {
            if (Random.rollPercent(100)) {
                player.animate(9158);
                player.graphics(1988, 0, 0);
                target.hit(new Hit(player, null, null).fixedDamage(1).ignoreDefence().ignorePrayer());
            }
        }

        /* Menacing Mage */
        if (hit.attackStyle != null && hit.attackStyle.isMagic() && player.SOTMM && player.getInventory().hasItem(26077, 1)) {
            if (Random.rollPercent(20)) {
                ...
                target.addEvent(event -> {
                    for (int i = 0; i < 6; i++) {
                        player.hit(new Hit(HitType.HEAL).fixedDamage(2));
                        event.delay(1);
                    }
                });
            }
    ...
    private final Bounds HOME = new Bounds(1726, 1559, 1795, 3632, 0);
    private final Bounds GLOBAL_BOSS = new Bounds(1759, 3560, 1792, 3585, 0);


    @Override
    public void startDeath(Hit killHit) {
        if (player.getPosition().inBounds(new Bounds(1759, 3560, 1792, 3585, 0))) {
            player.deathEndListener = (DeathListener.Simple) () -> {
                player.getMovement().teleport(1760, 3572, 0);
                player.sendMessage("Oh dear, you have died!");
            };
        }
        if (player.wildernessLevel <= 0 && player.deathEndListener == null) {
            if (player.getGameMode() == GameMode.IRONMAN || player.getGameMode() == GameMode.GROUP_IRONMAN || player.getGameMode() == GameMode.STANDARD) {
                player.deathEndListener = (DeathListener.Simple) () -> {
                    if (player.getPosition().regionId() == 11593) {
                        Nex.deadplayers.add(player);
                    }
                    player.getMovement().teleport(1702, 3746, 0);
                    player.sendMessage("Oh dear, you have died!");
                };
            }
        }
            ...
            if(player.deathEndListener != null) {
                ...
                if (useDeathStorage) {
                    ...
                    IKOD.forLostItem(player, killer, item -> {
                        ...
                        /**
                         * Uncharge charged items
                         */

                        if (item.hasAttributes())
                            item.clearAttributes();

                        if (pKiller == null || !def.tradeable && !pKiller.getGameMode().isIronMan())
    ...
    public void updateWeapon(boolean fromAutocast) {
        ...
        player.AutoCastStorer = weaponType.config == 18;
        if(!fromAutocast)
    ...
    public double getRegularMagicDamageBoost(boolean skipNightmareStaff) {
        ...
        if(equipment.wearsArcaneStreamNecklace()) {
            bonus += 0.15;
        }
    ...
    public double getDragonfireResistance() {
        ...
        if (player.getEquipment().wearsLavaDragonHideChest())
            absorbDamage += 0.40;
        if (player.getEquipment().wearsLavaDragonHideLegs())
            absorbDamage += 0.40;
        if (player.getEquipment().wearsLavaDragonHideHelm())
            absorbDamage += 0.20;
        if (player.pet == Pet.PRINCE_BLACK_DRAGON)
            absorbDamage += 1.0;
