...
//            ItemAction.registerEquipment(i, "check", PvPArmor::checkCharges);
    ...
    private static void postDamageListener(Player player, Item item, Hit hit) {
        ...
        if (AttributeExtensions.deincrementCharges(item, player.wildernessLevel > 0 ? 1 : 50) <= 0) {

            player.sendMessage(Color.RED.wrap("Your " + item.getDef().name + " has degraded to dust."));
            item.remove();
        }
        if (player.wildernessLevel <= 0 && player.PVPAmorNotification) {
            player.sendMessage(Color.RED,"While using PVP Items outside the wilderness you use 50x the normal rate of charges!");
            player.sendMessage(Color.RED,"To turn these notifications off do ;;pvparmour");
        }
    }
