    ...
    public static void printConfigs(int interfaceId, boolean recursiveSearch) {
        ...
        for(ScriptDef script : scripts) {
            for(int i = 0; i < script.instructions.length; i++) {
                ...
                if(instruction == 1)
                    varpIds.add(script.intOperands[i]);
                else if(instruction == 25) //get
                    varpIds.add(Varpbit.get(script.intOperands[i]).varpId);
            }
        }
        if(varpIds.isEmpty()) {
            System.out.println("Interface " + interfaceId + " uses no configs! [recursiveSearch=" + recursiveSearch + "]");
            System.out.println("    scripts=" + scripts.stream().map(def -> def.id).collect(Collectors.toList()));
            return;
        }
        ...
        for(ScriptDef script : scripts) {
            ...
            s: for(int i = 0; i < script.instructions.length; i++) {
                ...
                if(instruction == 40) {
                    /* include instruction */
                    if(recursiveSearch)
                        scripts.add(ScriptDef.get(script.intOperands[i]));
                } else if(instruction >= 1400 && instruction < 1500 || instruction >= 2400 && instruction < 2500) {
                    /* hook instruction */
                    int offset = i - 1;
                    if(instruction >= 2000)
                        offset--; //grabs interface
                    String string = script.stringOperands[offset--];
                    if(string == null)
                        string = "";
                    if(string.endsWith("Y")) {
                        int valueCount = script.intOperands[offset--];
                        offset -= valueCount;
                    ...
                    int scriptId = script.intOperands[offset];
                    if(scriptId != -1)
