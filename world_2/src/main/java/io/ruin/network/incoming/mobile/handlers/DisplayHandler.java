package io.ruin.network.incoming.mobile.handlers;
...
public class DisplayHandler implements Incoming {
    ...
    public void handle(Player player, InBuffer in, int opcode) {
        ...
        int canvasHeight = in.readShort();//gotta do 1 at a time lol ohh that makes sense now....xD
        System.out.println("PACKET DISPLAY MODE " + displayMode + " type " + player.getConfClientType());
    ...
    public static void setDisplayMode(Player p, int displayMode) {
        ...
        ps.sendGameFrame(getGameFrameFor(p, displayMode));
        ps.sendClientScript(3998, "i", displayMode - 1);
        System.out.println("Mode requested " + displayMode);
        switch (displayMode) {
            case 2:
        ...
        Config.varpbit(6347, false).set(p, 150); // quests completed
        Config.varpbit(11877, false).set(p, 150); // quests available

//        ps.sendClientScript(3970, "IIi", 46661634, 46661635, 60); // 0 = minutes played.
    ...
    public static void sendDisplay(Player player, int displayMode) {
        ...
        ps.sendInterface(WidgetID.PRIVATE_CHAT, Interface.DEFAULT_SCREEN, 23, 1);
...
/*        ps.sendAccessMask(WidgetID.OPTIONS_GROUP_ID, 106, 1, 4, 2);
    ...
    public static int getGameFrameFor(Player player, int displayMode) {
        if (player.getConfClientType() > 1) {
            return Interface.MOBILE_SCREEN;
        }
        switch (displayMode) {
            case 1:
